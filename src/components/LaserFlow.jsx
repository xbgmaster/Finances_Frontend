...
export const LaserFlow = ({
  ...
  // Start fully visible: the intro fade is frame-based and never completes when the
  // tab loads in the background (browsers pause requestAnimationFrame), which left the
  // beam invisible until the tab regained focus. Showing it from the first rendered
  // frame guarantees it's always there on initial load.
  ...
  useEffect(() => {
    ...
    try {
      ...
      // WebGL unavailable (disabled, blocklisted GPU, or too many live contexts
      // after repeated hot-reloads). Skip the effect gracefully; the CSS fallback
      // behind the login card keeps the page looking intentional.
      console.warn('[LaserFlow] WebGL is not available, skipping the animated background.', err);
      if (mount) mount.setAttribute('data-webgl', 'unavailable');
      return;
    }
    ...
    const animate = () => {
      ...
      // Keep rendering continuously while the tab is visible. We intentionally do
      // not gate on `inViewRef`: this is a full-screen fixed background, and gating
      // on intersection caused the loop to stall on initial mount (when the
      // container briefly had no size), leaving the beam invisible until a resize.
  ...
  return (
    <div
      ref={mountRef}
      className={className ? `laser-flow-container ${className}` : 'laser-flow-container'}
      style={style}
    />
  );
};
